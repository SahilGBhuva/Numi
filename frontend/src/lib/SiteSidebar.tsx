...
function Mark({ kind }: { kind: Screen }) {
  ...
  if (kind === 'quests') return <svg viewBox="0 0 32 32" aria-hidden="true"><path fill="#e0a045" d="M7 6h18v21l-9-3.4L7 27z" /><path fill="#c45c3c" d="M7 6h18v4H7z" /><rect fill="#f3e6c4" x="11" y="13.4" width="10" height="2" rx="1" /><rect fill="#f3e6c4" x="11" y="17.6" width="7" height="2" rx="1" /></svg>
  ...
  return (
    <nav className="bindit-rail" aria-label="Main">
      ...
      <ol className="bindit-rail__list">
        {ITEMS.map((item) => (
          <li key={item.id}><a className={`bindit-rail__item is-${item.id} ${active === item.id ? 'is-active' : ''}`} href={`#${item.id}`} aria-current={active === item.id ? 'page' : undefined}><span className="bindit-rail__icon"><Mark kind={item.id} /></span><span className="bindit-rail__label">{item.label}</span></a></li>
        ))}
      </ol>
    </nav>
  )
