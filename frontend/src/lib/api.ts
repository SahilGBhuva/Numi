...
async function request<T>(path: string, options?: RequestInit, accessToken?: string): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new Error(data?.detail ?? `Numi could not complete that request (${response.status}).`)
  }
...
export async function getProgress(studentId: string, accessToken?: string): Promise<Progress | null> {
  ...
  if (!response.ok) {
    const data = await response.json().catch(() => null) as { detail?: string } | null
    throw new Error(data?.detail ?? `Numi could not load progress (${response.status}).`)
  }
