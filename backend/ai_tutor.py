...
def generate_flashcards(*, course: str, unit: str, source_labels: list[str], count: int, personalization: dict[str, Any], source_text: str = "") -> list[dict[str, str]]:
    grounded = bool(source_text.strip())
    prompt = "You are Bindit's expert flashcard writer. Make high-value retrieval-practice cards. Avoid duplicates and trivia. " + ("Use the supplied note excerpts as primary ground truth and do not invent unsupported details. " if grounded else "Filenames are hints only. ") + "Return ONLY JSON {\"cards\":[{\"front\":string,\"back\":string,\"topic\":string}]} ."
...
def grade_answer(*, question: str, correct_answer: str, student_answer: str, topic: str, difficulty: int) -> dict[str, Any]:
    prompt = "You are Bindit's fast school tutor/grader. Use the reference as a rubric; accept equivalent wording and meaningful partial credit. Be concise. Return ONLY JSON with keys correct:boolean, score:0-100 integer, mistake_type:string|null, explanation:string, hint:string|null, misconception:string|null."
